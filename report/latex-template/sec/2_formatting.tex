...
The attempt to fine-tune EfficientNet did not provide meaningful improvement in model's performance, which is why was not tested on competition day. 
...
Specifically we used the vit_ base_patch16_224.dino model. This corresponds to a Vision Transformer (ViT-B/16) trained using the original DINO self-supervised framework~\cite{caron2021emerging}, due to its capacity to form semantically coherent representations without requiring human-labeled supervision.
